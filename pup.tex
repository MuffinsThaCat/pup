...
SHA-256 truncated to $n$ bytes). For domain separator $d \in \bin^8$
and address $\mathsf{addr} \in \bin^*$, define:
\[
\hash_n^d(\mathsf{addr}, x) = \hash(d \concat \mathsf{addr} \concat x)[{:}n]
\]
\end{definition}
...
is hashed to $n$ bytes, then encoded as $\ell_1 = \lceil 8n / \lg w
...
This model captures Ethereum (EIP-155~\cite{eip155}), Solana, Aptos,
Sui, and all account-based blockchains. UTXO-based chains (Bitcoin) do
not have per-account nonces and are outside our scope.
...
Given a random seed $\mathsf{sk} \xleftarrow{\$} \bin^{256}$:
...
$L_i = \hash_n^{\mathsf{LEAF}}(i,\, \mathsf{pk}_i^{(0)} \concat
\cdots \concat \mathsf{pk}_i^{(\ell-1)})$.
\item Build the Merkle tree over $\{L_i\}$. The root $R$ is the public
key.
\end{enumerate}

The full signing key is $(\mathsf{sk}, \text{tree})$. The public key is
$(R, \mathsf{params})$.
...
$\hat{m} = \mathsf{len}(m) \concat m \concat \Delta[0] \concat \cdots
\concat \Delta[|\Delta|-1]$.
...
Given public key $(R, \mathsf{params})$, message $m$, signature
$\Sigma = (i, \sigma_{\mathsf{ots}}, \Delta)$, and on-chain state
$\mathsf{state}$:
...
The expected delta count over $N = 2^k$ consecutive transitions is:
\[
\mathbb{E}[\delta] = 1 - \frac{1}{N}
\]
\end{theorem}

\begin{proof}
$\sum_{i=1}^{N} \nu_2(i) = N - s_2(N)$ where $s_2(N)$ is the number
of 1-bits in the binary representation of $N$. For $N = 2^k$,
$s_2(N) = 1$, giving $\sum = N - 1$. Thus
$\mathbb{E} = (N - 1) / N = 1 - 1/N$.
\end{proof}

\subsection{Signature Size}

The average signature size is:
\[
|\Sigma|_{\text{avg}} = 4 + \ell \cdot n + \mathbb{E}[\delta] \cdot n
= 4 + 35 \cdot 16 + 1 \cdot 16 = 580 \text{ bytes}
\]
...
(8, 3)\}$, covering all $17{,}100{,}032$ distinct digit pairs, and
by random sampling of $2^{20}$ message pairs at $(16, 32)$, with zero
...
security to ${\sim}2^{54.5}$. Since the OTS bound ($2^{54.9}$) remains
the bottleneck, the overall security level is unchanged. At $n = 20$,
the margin is $2.8$ bits and the multi-target cost is the same $3.0$
bits, giving effective collision security ${\sim}2^{70.5}$ vs.\ OTS
$2^{70.7}$---still OTS-dominated.
...
In Game~1 (all keys independent and random), the reduction answers all
$q'$ signing queries using the random keys at positions
$0, \ldots, q' - 1$. The reduction runs $\mathcal{A}$ with
\emph{honest} quantum oracle access to $\hash$---the random oracle is
not modified, patched, or reprogrammed. $\mathcal{A}$ makes
superposition queries to $\hash$ and receives honest answers throughout.
This is the critical structural property: because the reduction is
``straight-line'' (run $\mathcal{A}$ honestly, then extract from its
classical output), it avoids the oracle-reprogramming step that
introduces $O(q_H)$ or $O(q_H^2)$ losses in typical QROM
reductions~\cite{dongyanglong}.
...
Dong--Yang--Long~\cite{dongyanglong} measure-and-reprogram lemma
...
Public key & 32 B & 16 B & 16 B & 20 B \\
